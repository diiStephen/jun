...
instance Show TerminationError where 
    ...
    show (OtherError s) = "Could not prove termination: " ++ s
...
mpoTactic rule = do 
    ...
    case mpo sig (lhs rule) (rhs rule) of 
        GR -> return True 
        _  -> throwError (TFail ("Rule" ++ show rule ++ "canot be proved termination with mpo"))
...
checkTermination :: RewriteSystem -> TerminationEval Bool
checkTermination rs = do
    sig <- ask 
    lpoAttempt <- mapM lpoTactic (rules rs)
    return (and lpoAttempt)


evalTermination :: OrderedSig -> RewriteSystem -> (Either TerminationError Bool, [String])
evalTermination sig trs = runIdentity (runWriterT (runExceptT (runReaderT (checkTermination trs) sig)))